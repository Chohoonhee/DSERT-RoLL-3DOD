...
def project_lidar_to_image(lidar_points, extr, intr, image_shape):
    """
    lidar_points: (N, 3) or (N, 4) ndarray (x,y,z[,1])
    extr: (4,4) extrinsic matrix (T_cam<-lidar)
    intr: (3,3) intrinsic matrix
    image_shape: (H, W, C)

    return:
        projected: (M, 2) int array (pixel coords)
        depths:    (M,) float array (depths in camera frame)
    """
    H, W, _ = image_shape
    ...
    if lidar_points.shape[1] == 3:
        ...
        lidar_homo = lidar_points

    # (4,N) → 카메라 좌표계
    cam_points = (extr @ lidar_homo.T)[:3]  # (3,N)
    ...
    projected = np.round(proj[in_img]).astype(int)
