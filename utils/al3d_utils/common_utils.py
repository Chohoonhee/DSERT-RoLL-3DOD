...
def overlay_lidar_on_image(image, projected, depths, cmap='jet', vmin=None, vmax=None, point_size=2):
    """
    image: (H, W, 3) numpy array (BGR 또는 RGB)
    projected: (N,2) int array (u,v) pixel 좌표
    depths: (N,) float array (depth values, Z in camera)
    cmap: matplotlib colormap 이름
    vmin, vmax: depth 색상 범위 (None이면 depths.min()/max() 자동 사용)
    point_size: 이미지에 찍을 점 반경 (픽셀 단위)

    return: image_with_points (H, W, 3)
    """
    ...
    # depth → 0~1 정규화
    norm_depths = np.clip((depths - vmin) / (vmax - vmin + 1e-6), 0, 1)

    # jet 컬러맵 적용
    cmap = plt.get_cmap(cmap)
    colors = (cmap(norm_depths)[:, :3] * 255).astype(np.uint8)  # RGBA → RGB

    # 복사본 이미지
    img_vis = image.copy()

    # 점 찍기
    for (u, v), c in zip(projected, colors):
        # pdb.set_trace()
        if 0 <= u < img_vis.shape[1] and 0 <= v < img_vis.shape[0]:
            cv2.circle(img_vis, (int(u), int(v)), point_size, color=(int(c[0]), int(c[1]), int(c[2])), thickness=-1)
