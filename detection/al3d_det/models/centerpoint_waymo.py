...
class CenterPointPC(nn.Module):
    ...
    def build_networks(self):
        model_info_dict = {
            ...
            'num_point_features_radar': self.dataset.point_feature_encoder_radar.num_point_features,
        ...
        vfe_radar = backbone_3d.__all__[self.model_cfg.VFE_RADAR.NAME](
            model_cfg=self.model_cfg.VFE_RADAR,
            num_point_features=model_info_dict['num_point_features_radar'],
            point_cloud_range=model_info_dict['point_cloud_range'],
            voxel_size=model_info_dict['voxel_size'],
            grid_size=model_info_dict['grid_size'],
        )
        model_info_dict['num_point_features_radar'] = vfe_radar.get_output_feature_dim()
        ...
        backbone3d_radar = backbone_3d.__all__[self.model_cfg.BACKBONE_3D_RADAR.NAME](
            model_cfg=self.model_cfg.BACKBONE_3D_RADAR,
            input_channels=model_info_dict['num_point_features_radar'],
            grid_size=model_info_dict['grid_size'],
            voxel_size=model_info_dict['voxel_size'],
            point_cloud_range=model_info_dict['point_cloud_range'],
        )
        model_info_dict['num_point_features_radar'] = backbone3d_radar.num_point_features
        ...
        map_to_bev_radar = backbone_2d.__all__[self.model_cfg.MAP_TO_BEV_RADAR.NAME](
            model_cfg=self.model_cfg.MAP_TO_BEV_RADAR,
            grid_size=model_info_dict['grid_size']
        )
        model_info_dict['num_bev_features_radar'] = map_to_bev_radar.num_bev_features
        ...
        backbone2d_radar = backbone_2d.__all__[self.model_cfg.BACKBONE_2D_RADAR.NAME](
            model_cfg=self.model_cfg.BACKBONE_2D_RADAR,
            input_channels=model_info_dict['num_bev_features_radar']
        )
        model_info_dict['num_bev_features_Radar'] = backbone2d_radar.num_bev_features

        lidar_radar_union = backbone_3d.__all__[self.model_cfg.LIDAR_RADAR_UNION.NAME](
            model_cfg=self.model_cfg.LIDAR_RADAR_UNION
        )
        ...
        self.add_module('vfe_radar', vfe_radar)
        self.add_module('backbone3d', backbone3d)
        self.add_module('map_to_bev', map_to_bev)
        self.add_module('backbone2d', backbone2d)
        self.add_module('backbone3d_radar', backbone3d_radar)
        self.add_module('map_to_bev_radar', map_to_bev_radar)
        self.add_module('backbone2d_radar', backbone2d_radar)
        self.add_module('lidar_radar_union', lidar_radar_union)
        

        self.add_module('dense_head', dense_head)
        module_list = [vfe, vfe_radar, backbone3d, map_to_bev, backbone2d, backbone3d_radar, map_to_bev_radar, backbone2d_radar, lidar_radar_union,  dense_head]
    ...
    def post_processing(self, batch_dict):
        ...
        # selected_score = batch_dict['roi_scores'] # B x n
        # selected_boxes = batch_dict['rois'] # B x n x 7
        # selected_labels = batch_dict['roi_labels'] # B x n
        ...
        if self.second_stage:
            ...
            for index in range(batch_size):
                ...
                if post_process_cfg.NMS_CONFIG.MULTI_CLASSES_NMS:
                    ...
                    # box_preds = selected_boxes[index]
                    # scores = selected_score[index]
                    # label_preds = selected_labels[index]
