...
class CenterPointMM_Camera(nn.Module):
    def __init__(self, model_cfg):
        ...
        if 'THERMAL_BACKBONE' in model_cfg:
            self.thermal_backbone = img_modules.__all__[model_cfg.THERMAL_BACKBONE.NAME](
                model_cfg=model_cfg.THERMAL_BACKBONE
            )
        else:
            self.thermal_backbone = None
            
        if 'EVENT_BACKBONE' in model_cfg:
            self.event_backbone = img_modules.__all__[model_cfg.EVENT_BACKBONE.NAME](
...
class CenterPointMM(nn.Module):
    ...
    def forward(self, batch_dict):
        #####################################
        # Not use, use viz in loader (dataset.py L129)
        # import pdb; pdb.set_trace()
        # import cv2
        # pcd = batch_dict['points'][batch_dict['points'][:, 0] == 0][:, 1:].detach().cpu().numpy()
        # image = batch_dict['images']['camera_0'][0].detach().cpu().numpy()*255
        # extr = batch_dict['extrinsic']['camera_0'][0].detach().cpu().numpy() 
        # intr = batch_dict['intrinsic']['camera_0'][0].detach().cpu().numpy() 
        # points_img, points_depth = common_utils.lidar_to_image_strict(pcd, extr, intr)
        # out = common_utils.overlay_lidar_on_image(image, points_img[:, ::-1], points_depth[:, 0], vmin=0, vmax=30, point_size=10)
        # cv2.imwrite('out.png', out.transpose(1, 2, 0))
        #########################################
        
        batch_dict = self.camera.img_backbone(batch_dict)
        if self.camera.thermal_backbone is not None:
            batch_dict = self.camera.thermal_backbone(batch_dict)
        
        if self.camera.event_backbone is not None:
            batch_dict = self.camera.event_backbone(batch_dict)
        ...
        batch_dict = self.lidar(batch_dict, self.lidar.module_list[6])
        batch_dict = self.lidar(batch_dict, self.lidar.module_list[7])
        batch_dict = self.lidar(batch_dict, self.lidar.module_list[8])
        batch_dict = self.lidar(batch_dict, self.lidar.module_list[9])
        batch_dict = self.lidar(batch_dict, self.lidar.module_list[10])
