...
class DatasetTemplate(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None) -> None:
        ...
        self.point_feature_encoder_radar = PointFeatureEncoder(
    ...
    def __getitem__(self, index):
        ...
        target_infos, points, points_radar = self.get_infos_and_points(target_idx_list)
        # import pdb; pdb.set_trace()
        if len(points) == 1: 
            points = points[-1]
            points_radar = points_radar[-1]
        else:
            points = points[-1]
            points_radar = self.merge_sweeps(current_info, target_infos, points_radar, merge_multiframe = self.merge_multiframe)
        input_dict = {
            'points': points,
        ...
        if self.use_image:
            ...
            # if True:
                ...
                for cam in img_dict['images'].keys():
                    ...
                    cv2.imwrite('../../%s.jpg' % cam, img)

        if 'annos' in current_info:
            annos = current_info['annos']
            annos = common_utils.drop_info_with_name(annos, name='unknown')

            # print(current_info['sequence_name'], current_info['sample_idx'])

            if self.dataset_cfg.get('INFO_WITH_FAKELIDAR', False):
                gt_boxes_lidar = box_utils.boxes3d_kitti_fakelidar_to_lidar(annos['gt_boxes_livox'])
            else:
                gt_boxes_lidar = annos['gt_boxes_livox']
    ...
    def merge_sweeps(info, target_infos, points, merge_multiframe=False,
                    keep_extra_fields=True, add_time_offset=False, time_unit='us'):
        """
        여러 sweep의 point cloud를 현재 프레임 좌표계로 변환/정렬하여 병합.

        Args:
            info: dict, {'pose': (4,4), 'time_stamp': int}
            target_infos: list of dicts, 각 원소는 {'pose': (4,4), 'time_stamp': int}
            points: list of np.ndarray, 각 원소 shape (Ni, K), K>=3, 앞 3개는 xyz, 나머지는 부가필드
            merge_multiframe: (미사용) 멀티프레임 병합 플래그
            keep_extra_fields: True면 xyz 이후 컬럼(예: intensity, ring 등) 유지
            add_time_offset: True면 delta_time 컬럼(타겟 − 현재)을 추가
            time_unit: 'us' | 'ms' | 's'  (delta_time 스케일)

        Returns:
            point_clouds: (sum_i Ni, K [+1 if add_time_offset]) ndarray
        """
        current_pose = info["pose"]
        current_time = int(info["time_stamp"])
        ...
        for i in range(len(target_infos)):
            ...
            cur_pts = np.asarray(points[i])
            assert cur_pts.ndim == 2 and cur_pts.shape[1] >= 3, "points must be (N, K>=3)"

            # (N,3) xyz만 뽑아서 동차 좌표로 확장
            xyz = cur_pts[:, :3]
            ones = np.ones((xyz.shape[0], 1), dtype=xyz.dtype)
            xyz_h = np.hstack([xyz, ones])  # (N,4)

            # 타겟 → 현재 프레임으로 좌표 변환
            transform_mat = np.linalg.inv(current_pose) @ target_info['pose']  # (4,4)
            xyz_h_out = (transform_mat @ xyz_h.T).T  # (N,4)
            xyz_out = xyz_h_out[:, :3] / xyz_h_out[:, 3:4]  # 원근(w) 고려

            # 나머지 부가 필드 보존
            if keep_extra_fields and cur_pts.shape[1] > 3:
                out = np.hstack([xyz_out, cur_pts[:, 3:]])
            else:
                out = xyz_out

            # delta_time 추가 (옵션)
            if add_time_offset:
                delta_time = (int(target_info['time_stamp']) - current_time) / denom
                dt_col = np.full((out.shape[0], 1), delta_time, dtype=out.dtype)
                out = np.hstack([out, dt_col])

            merged.append(out)

        point_clouds = np.concatenate(merged, axis=0)
        return point_clouds
    ...
    def collate_batch(batch_list, _unused=False):
        ...
        for key, val in data_dict.items():
            ...
            try:
                ...
                elif key in ['points', 'voxel_coords', 'voxel_coords_downscale', 'points_radar', 'voxel_coords_radar']:
                    coors = []
