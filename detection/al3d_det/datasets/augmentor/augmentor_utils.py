...
def random_flip_along_x(gt_boxes, points, points_radar, return_enable_xy=False):
    """
    Args:
    ...
    if enable:
        ...
        points[:, 1] = -points[:, 1]
        points_radar[:, 1] = -points_radar[:, 1]
        

        if gt_boxes.shape[1] > 7:
            gt_boxes[:, 8] = -gt_boxes[:, 8]
    if return_enable_xy:
        return gt_boxes, points, points_radar, int(enable)
    return gt_boxes, points, points_radar


def random_flip_along_y(gt_boxes, points, points_radar, return_enable_xy=False):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C)
...
def global_rotation(gt_boxes, points, points_radar, rot_range, return_rotate_noise=False):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C),
    ...
    points_radar = common_utils.rotate_points_along_z(points_radar[np.newaxis, :, :], np.array([noise_rotation]))[0]
    ...
    if return_rotate_noise:
        return gt_boxes, points, points_radar, noise_rotation
    return gt_boxes, points, points_radar
...
def global_scaling(gt_boxes, points, points_radar, scale_range, return_scale_noise=False):
    """
    Args:
    ...
    points_radar[:, :3] *= noise_scale
    gt_boxes[:, :6] *= noise_scale

    if gt_boxes.shape[1] > 7:
        gt_boxes[:, 7:9] *= noise_scale
    if return_scale_noise:
        return gt_boxes, points, points_radar, noise_scale
    return gt_boxes, points, points_radar
...
def global_translation(gt_boxes, points, points_radar, std, return_std_noise=False):
    ...
    points_radar[:, 0] += x_trans
    points_radar[:, 1] += y_trans
    points_radar[:, 2] += z_trans
    ...
    gt_boxes[:, 2] += z_trans
    if return_std_noise:
        return gt_boxes, points, points_radar, np.array([x_trans, y_trans, z_trans]).T
    return gt_boxes, points, points_radar
